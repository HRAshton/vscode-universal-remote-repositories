...
Copy `dist/bitbucket-vscode-workbench-relay.js` into the workbench's static files and load it from the
top-level workbench HTML before the workbench bootstrap:

```html
<script src="./bitbucket-vscode-workbench-relay.js"></script>
```
...
private message channel tied to that exact child window and the configured local origin. The top-level relay
forwards the typed protocol to the extension-host worker over a capability-scoped `BroadcastChannel`.
